...
impl Writer {
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            ...
            byte => {
                ...
                let col: usize = self.column_position.clone();
    ...
    fn clear_row(&mut self, row: usize) {
        for col in 0..BUFFER_WIDTH {
            self.buffer.chars[row.clone()][col].write(ScreenChar {
                ascii_character: b' ',
...
fn test_println_output() {
    ...
    interrupts::without_interrupts(|| {
        ...
        // This way, we avoid test failure when the timer handler has already
        // printed some . characters to the current line
